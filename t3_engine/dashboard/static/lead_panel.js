...
(function (global) {
  ...
  Panel.prototype.collect = function (scope) {
    var self = this;
    if (!scope) this.nodes = {};
    Array.prototype.forEach.call((scope || this.root).querySelectorAll('[data-le]'), function (el) {
  ...
  Panel.prototype.schedule = function () {
    ...
    this.timerHandle = setTimeout(function () {
      ...
      self.frameHandle = requestAnimationFrame(function () {
        ...
        var frame = self.pending; self.pending = null;
        if(!frame) return;
        self.lastPaint = performance.now(); self.paints += 1;
        self.displayLatencyMs = self.lastPaint - self.pendingAt;
        self.paint(frame);
      });
    }, Math.max(0, this.refreshMs - (performance.now() - this.lastPaint)));
  };
  ...
  Panel.prototype.stats = function () {
    return { paints: this.paints, writes: this.writes, skipped: this.skipped,
             refreshMs: this.refreshMs, pendingFrames:this.pending ? 1 : 0, displayLatencyMs:this.displayLatencyMs };
  };
