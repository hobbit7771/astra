...
(function (global) {
  ...
  function FibTool(options) {
    ...
    this.ratios = (options.ratios || RETRACEMENTS).slice();
    this._loaded = false;
  ...
  FibTool.prototype.save = function () {
    ...
    try {
      var drawings = this.drawings.map(function (d) {
        return { id: d.id, start: d.start, end: d.end, hidden: d.hidden };
      });
      localStorage.setItem(this.key(), JSON.stringify({version:2, drawings:drawings, ratios:this.ratios, hidden:this.hidden}));
  ...
  FibTool.prototype.load = function () {
    ...
    this._loaded = true;
    if (!raw) { this.ratios = RETRACEMENTS.slice(); this.hidden = false; this.onChange(this); return; }
    var payload;
    try { payload = JSON.parse(raw) || []; } catch (e) { payload = []; }
    var self = this;
    var drawings = Array.isArray(payload) ? payload : (payload.drawings || []);
    this.ratios = !Array.isArray(payload) && Array.isArray(payload.ratios)
      ? payload.ratios.filter(function(r){return ALL.indexOf(r)!==-1;}) : RETRACEMENTS.slice();
    this.hidden = !Array.isArray(payload) && !!payload.hidden;
    (Array.isArray(drawings) ? drawings : []).slice(-20).forEach(function (d) {
      if (!d || !validPoint(d.start) || !validPoint(d.end)) return;
  ...
  FibTool.prototype.setRatios = function (ratios) {
    this.ratios = (ratios || []).filter(function(r){return ALL.indexOf(r)!==-1;}).sort(function (a, b) { return a - b; });
    this.save();
    this.redraw();
  };
  ...
  function validPoint(p) {return p && Number.isFinite(p.price) && p.price > 0 && Number.isFinite(p.time) && p.time > 0;}
  FibTool.prototype.addPoint = function (point) {
    if (!validPoint(point)) return;
    if (!this.pendingPoint) { this.pendingPoint = point; this.onChange(this); return; }
    if (this.pendingPoint.price === point.price) return;
    var drawing = {
      id: 'fib-' + Date.now(),
    ...
    this.drawings = this.drawings.slice(-20);
    this.arm(false);
    this.save();
    this.redraw();
  };
  ...
  FibTool.prototype._point = function (clientX, clientY) {
    ...
    if (price === null || price === undefined || !Number.isFinite(time)) return null;
  ...
  FibTool.prototype._bind = function () {
    ...
    var startX = 0, startY = 0, startedAt = 0, tap = false;
    this.container.addEventListener('touchstart', function (event) {
      tap = self.armed && event.touches.length === 1;
      if (!tap) return;
    ...
    this.container.addEventListener('touchmove', function(event){
      if(event.touches.length !== 1 || Math.abs(event.touches[0].clientX-startX)+Math.abs(event.touches[0].clientY-startY)>TAP_SLOP) tap=false;
    }, {passive:true});
    this.container.addEventListener('touchcancel', function(){tap=false;}, {passive:true});
    this.container.addEventListener('touchend', function (event) {
      if (!self.armed || !tap || event.touches.length) return;
      tap = false;
