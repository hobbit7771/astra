...
(function (global) {
  ...
  function buildBlocks(panel) {
    ...
    html += card('Astra · PAPER portfolio', p.row('paper:realized','Closed P&L · USDT') + p.row('paper:open','Open P&L · USDT') + p.row('paper:count','Closed / winners') + '<a href="/paper">Trade journal</a>');
    ...
    ['short', 'long'].forEach(function (side) {
      var rows = ['compression', 'fading_bounces', 'depth_drain', 'defender_pulling',
                  ...
                  'velocity_rising', 'btc_alignment', 'repeated_tests', 'normalized_cvd', 'deep_obi', 'liquidation_cascade']
        .map(function (name) {
  ...
  function applyFrame(panel, frame, extra) {
    ...
    var p = panel;
    var paper=frame.paper || {};
    p.set('paper:realized',fmt(paper.realized_net_pnl,4));
    p.set('paper:open',fmt(paper.unrealized_net_pnl,4));
    p.set('paper:count',paper.closed_count === undefined ? '—' : paper.closed_count+' / '+paper.wins);
    ...
    p.set('sig:score', fmt(sig.model_score === undefined ? sig.break_probability : sig.model_score, 1));
    ...
    var clockLimits = { ws: 2000, book: 1000, trade: 1500, ticker: 30000,
