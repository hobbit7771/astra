...
def _combine(parts: Dict[str, Optional[float]], weights: Dict[str, float],
    ...
    available = {k: v for k, v in parts.items() if v is not None and math.isfinite(v)}
...
def score_flow(flow, cvd, normalizer: Normalizer) -> LayerScore:
    ...
    # has been up.
    # Preserve the sign of aggression. A mean-centred z-score of +100
    # after +1000 is negative, but +100 is still buying, not selling.
    total_volume = window_60s.buy_volume + window_60s.sell_volume
    parts["cvd_slope"] = (clamp(window_60s.delta / total_volume)
...
def absorption_scores(book, normalizer: Normalizer) -> Dict[str, float]:
    ...
    for side in ("bid", "ask"):
        ...
        # L2 is aggregated: this is an absorption estimate, not proof of
        # an individual order. Require BOTH executions and replenishment.
        matched = max(0.0, min(added, executed))
        vs_volume = matched / executed if executed > 0 else 0.0
        vs_depth = matched / depth if depth > 0 else 0.0
        percentile = normalizer.update(f"{side}_absorption", vs_depth, "percentile")
        zscore = normalizer.feature(f"{side}_absorption").zscore(vs_depth)
        magnitude = saturate(vs_depth, 0.25)
        out.update({
            f"{side}_absorption_vs_volume": vs_volume,
            f"{side}_absorption_vs_depth": vs_depth,
            f"{side}_absorption_zscore": zscore * 3.0,
            f"{side}_absorption_percentile": percentile,
            f"{side}_absorption_score": clamp(vs_volume * magnitude *
                                                (0.5 + 0.5 * percentile), 0.0, 1.0),
        })
    return out
...
def score_book(book, microprice, normalizer: Normalizer) -> LayerScore:
    ...
    half_spread = (metrics.spread_bps or 0.0) / 2.0
    parts["microprice"] = (clamp(offset / half_spread) *
                           min(1.0, 0.15 + abs(alignment["deep_book_score"]))
                           if offset is not None and half_spread > 0 else None)
    bid_side = metrics.bid_replenishment - metrics.bid_pulling
    ask_side = metrics.ask_replenishment - metrics.ask_pulling
    parts["liquidity_shift"] = clamp(
        5.0 * (bid_side / max(metrics.bid_depth, 1e-9) -
               ask_side / max(metrics.ask_depth, 1e-9)))
...
def score_structure(smc_state, elliott_component: Optional[float],
    ...
    if price and high and low and high > low and trend in ("bullish", "bearish"):
        sign = 1.0 if trend == "bullish" else -1.0
        distance = (price - low) if sign > 0 else (high - price)
        parts["invalidation_distance"] = sign * clamp(distance / (high - low), 0.0, 1.0)
    else:
...
def score_derivatives(open_interest, liquidations, ticker: Dict[str, Any],
    ...
    oi_change = open_interest.delta_pct()
    price_change = open_interest.price_change()
    parts["open_interest"] = None
    if oi_change is not None and price_change is not None:
        # Relative to the median absolute OI change; sign comes from price,
        # never from a centred z-score. Falling OI is weaker continuation.
        magnitude = normalizer.update("oi_change_magnitude", abs(oi_change), "ratio")
        if normalizer.ready("oi_change_magnitude"):
            sign = 1.0 if price_change > 0 else -1.0 if price_change < 0 else 0.0
            parts["open_interest"] = sign * magnitude * (1.0 if oi_change > 0 else 0.5)
    recent = liquidations.window(5_000)
    long_volume, short_volume = recent["long_notional"], recent["short_notional"]
    velocity = recent["velocity"]
    magnitude = normalizer.update("liquidation_velocity", velocity, "ratio")
    parts["liquidations"] = (normalized_delta(short_volume, long_volume) * magnitude
                             if normalizer.ready("liquidation_velocity") else None)
    detail_extra["liquidation_velocity_percentile"] = normalizer.feature(
        "liquidation_velocity").percentile(velocity)
    ...
    if funding is not None:
        magnitude = normalizer.update("funding_magnitude", abs(funding), "ratio")
        if normalizer.ready("funding_magnitude"):
            parts["funding"] = -math.copysign(magnitude, funding) if funding else 0.0
...
def score_btc_lead(lead_state: Dict[str, Any], btc_flow_score: Optional[float],
    ...
    parts: Dict[str, Optional[float]] = {
        "lead_score": (lead_state.get("btc_lead_score")
                       if lead_state.get("correlation") is not None else None),
...
def detect_conflict(layers: Dict[str, LayerScore]) -> Conflict:
    ...
    scores = [layer.score * layer.confidence for layer in live.values()]
