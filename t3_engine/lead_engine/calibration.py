...
class Calibrator:
    ...
    opened = 0
    abandoned = 0
    last_opened: Dict[str, int] = field(default_factory=dict)
    ...
    def observe(self, direction: str, score: float, level: Optional[float],
        ...
        if score < MIN_SCORE_TRACKED or not level or not price or price <= 0:
            return None
        if timestamp_ms - self.last_opened.get(direction, -max(HORIZONS_MS)) < max(HORIZONS_MS):
            return None
        ...
        self.last_opened[direction] = int(timestamp_ms)
        self.opened += 1
        return observation

    def invalidate_pending(self) -> None:
        """An outage is an unobserved outcome, not a failed prediction."""
        self.abandoned += len(self.pending)
        self.pending.clear()
