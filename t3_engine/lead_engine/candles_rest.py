...
def fetch_candles(symbol: str, interval: str, limit: int = DEFAULT_LIMIT,
    ...
    try:
        ...
        if payload.get("retCode", 0) != 0:
            return []
        exchange_ms = int(payload.get("time") or 0)
    ...
    for row in rows:
        try:
            ...
            continue
    out = [c for c in out if c['time'] > 0 and all(math.isfinite(c[k]) and c[k] > 0
            for k in ('open','high','low','close')) and math.isfinite(c['volume']) and c['volume'] >= 0
            and c['low'] <= min(c['open'],c['close']) and c['high'] >= max(c['open'],c['close'])]
    ...
    if exchange_ms:
        for c in out:
            c["closed"] = (c["time"] + seconds) * 1000 <= exchange_ms
    elif out:
        out[-1]["closed"] = False
