...
class StreamHealth:
    ...
    def book_age_ms_now(self, now_ms: Optional[int] = None) -> Optional[float]:
        """Age of the exchange event, including delivery and queueing."""
        return self.age_ms(self.last_book_ms, now_ms)

    def trade_age_ms_now(self, now_ms: Optional[int] = None) -> Optional[float]:
        return self.age_ms(self.last_trade_ms, now_ms)
    ...
    def as_dict(self, now_ms: Optional[int] = None) -> Dict[str, object]:
        return {
            ...
            "queue_latency_ms": round(self.queue_latency_ms, 3),
            "processing_backlog": self.processing_backlog,
            "history_truncated": self.history_truncated,
            "book_message_age_ms": self.age_ms(self.last_book_receive_ms, now_ms),
            "trade_message_age_ms": self.age_ms(self.last_trade_receive_ms, now_ms),
...
def assess(health: StreamHealth, thresholds: Thresholds,
    ...
    # opinion at all. Both use exchange data age, not just arrival age.
    degraded = False
    ...
    if trade_age is None:
        reasons.append("no trades received yet")
        mute = True
    elif trade_age > thresholds.trade_age_signals_off_ms:
        reasons.append(f"no trade for {trade_age / 1000.0:.1f}s "
                       f"(> {thresholds.trade_age_signals_off_ms / 1000.0:.1f}s)")
        mute = True

    if health.history_truncated:
        reasons.append("trade history capacity exceeded; rolling windows incomplete")
        mute = True
    if health.processing_backlog or health.queue_latency_ms > thresholds.max_queue_latency_ms:
        reasons.append(f"processing backlog: {health.queue_latency_ms:.0f}ms")
        mute = True
    if health.processing_latency_ms > thresholds.max_processing_latency_ms:
        reasons.append(f"processing too slow: {health.processing_latency_ms:.0f}ms")
        mute = True
    stamp = int(time.time() * 1000) if now_ms is None else now_ms
    if any(t > stamp + 1000 for t in (health.last_book_ms, health.last_trade_ms)):
        reasons.append("exchange clock ahead of server; freshness uncertain")
        mute = True
