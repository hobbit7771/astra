...
class TimeSeries:
    ...
    def __post_init__(self) -> None:
        ...
        self.reference_ms: Optional[int] = None
        self.truncated_until_ms = 0
    ...
    def _trim_locked(self) -> None:
        ...
        while len(self._items) > self.max_items:
            stamp, _ = self._items.popleft()
            self.truncated_until_ms = max(self.truncated_until_ms, stamp + self.horizon_ms)
    ...
    def window(self, window_ms: int, now_ms: Optional[int] = None) -> List[object]:
        ...
        with self._lock:
            if not self._items:
                return []
            reference = (self.reference_ms if self.reference_ms is not None
                         else self._items[-1][0]) if now_ms is None else int(now_ms)
