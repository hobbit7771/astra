...
class Normalizer:
    ...
    def update(self, name: str, value: Optional[float], method: str = "zscore",
        ...
        key = getattr(self, "sample_key", None)
        if key is None or getattr(rolling, "last_sample_key", None) != key:
            rolling.observe(number)
            rolling.last_sample_key = key
...
def normalized_delta(buy_volume: float, sell_volume: float,
    ...
    imbalance."""
    buy, sell = float(buy_volume), float(sell_volume)
    if not all(math.isfinite(v) and v >= 0 for v in (buy, sell)):
        return 0.0
    # Scale first: even two finite float volumes can overflow their sum.
    scale = max(buy, sell, max(epsilon, 1e-300))
    return clamp((buy / scale - sell / scale) /
                 (buy / scale + sell / scale + max(epsilon, 1e-300) / scale))
