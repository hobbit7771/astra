...
Bybit's `allLiquidation.S` is the POSITION side: Buy means a liquidated
long, Sell a liquidated short. This differs from publicTrade.S (taker
side). See https://bybit-exchange.github.io/docs/v5/websocket/public/all-liquidation.
...
def side_is_long_liquidation(side: str) -> bool:
    """Bybit position side, NOT the closing order's side."""
...
class LiquidationEngine:
    ...
    def acceleration(self) -> float:
        ...
        newest = self.events.reference_ms or self.events.newest_timestamp()
        if newest is None:
            return 0.0
...
def liquidation_from_message(item: Dict, symbol: str) -> Optional[Liquidation]:
    ...
    try:
        ...
        return None
    if not all(math.isfinite(v) and v > 0 for v in (price, quantity, stamp)):
        return None
