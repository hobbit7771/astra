...
class ElliottContext:
    ...
    def context(self) -> WaveContext:
        ...
        anchor = max(0, len(legs) - len(SEQUENCE))
        rejected = 0
        ...
        if not counted or _violates_hard_rules(counted):
            return WaveContext(symbol=self.symbol, resets=rejected,
        ...
        return WaveContext(symbol=self.symbol, label=label, phase=phase,
                           ...
                           resets=rejected, source="internal", note=note)
