...
Bybit publishes snapshots followed by ordered deltas. Update IDs advance,
but are not documented to be consecutive integers. Transport loss and
reconnect explicitly reset the book; snapshots replace every level/cache.
...
class Wall:
    ...
    def observe(self, size: float, timestamp_ms: int) -> None:
        ...
        if size < previous * 0.8:
            self.reductions += 1
        elif size > previous * 1.2 and self.reductions > self.replenishments:
...
class OrderBook:
    ...
    def reset(self, reason: str = "resync") -> None:
        ...
        self._walls.clear()
        self._sorted_version = None
        self._sorted_bids = []
        self._sorted_asks = []
        self._flows = TimeSeries(horizon_ms=FLOW_WINDOW_MS * 4)
    ...
    def apply(self, message: Dict) -> bool:
        ...
        Valid snapshots replace all state. Deltas need increasing IDs;
        Bybit does not promise u+1. Queue loss explicitly calls reset()."""
        ...
        try:
            ...
            self.reset("malformed orderbook envelope")
            return False

        if stamp <= 0 or update_id <= 0:
            self.reset("invalid book timestamp or update ID")
            return False
        for side in ("b", "a"):
            try:
                for row in data.get(side, []):
                    p, q = float(row[0]), float(row[1])
                    if not math.isfinite(p) or not math.isfinite(q) or not math.isfinite(p * q) or p <= 0 or q < 0:
                        raise ValueError("invalid level")
            except (ValueError, TypeError, IndexError):
                self.reset("malformed orderbook level")
                return False
        if kind == "snapshot" or update_id == 1:
            ...
            self.snapshots += 1
            if not self.bids or not self.asks or self._crossed():
                self.crossings += 1
                self.synced = False
                self.desync_reason = "snapshot empty or crossed"
                return False
            self.synced = True
            self.desync_reason = ""
        ...
        if not self.bids or not self.asks or self._crossed():
            ...
            self.crossings += 1
            self.synced = False
            self.desync_reason = "empty or crossed book after delta"
            return False
    ...
    def _track_walls(self, stamp: int) -> None:
        ...
        if threshold > 0:
            for side, levels in (("bid", self.top_bids()), ("ask", self.top_asks())):
                for price, size in levels:
                    key = (side, price)
                    if size < threshold and key not in self._walls:
                        continue
        ...
        for key, wall in self._walls.items():
            ...
            if wall.price not in (self.bids if wall.side == "bid" else self.asks) and not self._price_reached(wall) and wall.executed <= 0:
    ...
    def side_totals(self) -> Tuple[float, float]:
        ...
        return (sum(size for _, size in self.top_bids()), sum(size for _, size in self.top_asks()))
    ...
    def obi(self, depth: int) -> float:
        ...
        total = bids + asks
        return normalized_delta(bids, asks)
    ...
    def weighted_obi(self) -> float:
        ...
        return normalized_delta(bid_weighted, ask_weighted)
    ...
    def microprice(self) -> Optional[float]:
        ...
        if total <= 0:
            return bid + (ask - bid) / 2.0
        return bid + (ask - bid) * (1 + normalized_delta(bid_size, ask_size)) / 2
    ...
    def metrics(self, window_ms: int = FLOW_WINDOW_MS) -> OrderBookMetrics:
        ...
        out.imbalance = normalized_delta(out.bid_depth, out.ask_depth)
    ...
    def wall_summary(self, now_ms: Optional[int] = None) -> Dict[str, object]:
        ...
        mid = self.midpoint()
        for wall in walls:
            wall["distance_from_mid_bps"] = (abs(float(wall["price"]) - mid) / mid * 10_000
                                             if mid else None)
        ...
        # Unweighted denominator preserves the persistence discount.
        total = sum(float(w["size"]) for w in walls)
        return {
            ...
            "recent_cancelled": [{**w.as_dict(now_ms), "classification": POSSIBLE_SPOOF,
                                  "weight": 0.0}
                                 for w in self._spoofs.window(60_000)[-10:]],
        }
