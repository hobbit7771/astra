...
# the hard cap bounds memory. The separate 250ms age limit prevents
# consuming stale backlog even when the capacity has not been reached.
QUEUE_LIMIT = 2048
MAX_QUEUE_AGE_MS = 250.0
...
class StreamStats:
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "messages_per_second": round(self.messages_per_second, 1) if time.time() - self.last_message_at < 2 else 0.0,
...
class BybitLeadStream:
    ...
    def _demand_resync(self, symbols: Iterable[str], reason: str) -> None:
        ...
        # The queue interleaves symbols. Discarding any backlog resets all
        # books, since the dropped frames may belong to any instrument.
        wanted = list(self.symbols) if list(symbols) else []
        if not wanted:
            return
        with self._resync_lock:
    ...
    def _receive(self, raw, received_at: float) -> None:
        ...
        try:
            self._queue.put_nowait((raw, received_at, self._generation))
        except queue.Full:
    ...
    def _drain(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if item is None:                     # the stop sentinel
                break
            raw, received_at = item[:2]
            generation = item[2] if len(item) > 2 else self._generation
            waited_ms = max(0.0, (time.time() - received_at) * 1000.0)
            self.stats.note_queue_wait(waited_ms)
            if waited_ms > MAX_QUEUE_AGE_MS:
                self.stats.dropped += 1
                self._demand_resync(self.symbols, "processing backlog")
                continue
            with self._delivery_lock:
    ...
    def _handle(self, raw, received_at: Optional[float] = None) -> None:
        try:
            ...
            self.stats.dropped += 1
            self._demand_resync(self.symbols, "unreadable message")
            return
        if not isinstance(message, dict):
            self.stats.dropped += 1
            self._demand_resync(self.symbols, "invalid message envelope")
            return
        ...
        message["_received_at_ms"] = int(received_at * 1000)
        message["_queue_wait_ms"] = max(0.0, time.time() * 1000 - received_at * 1000)
        try:
            ...
            self.stats.dropped += 1
            if str(topic).startswith("orderbook."):
                self._demand_resync([parse_topic(str(topic))["symbol"]], "book handler failed")
