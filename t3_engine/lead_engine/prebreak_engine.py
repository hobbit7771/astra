...
FEATURE_WEIGHTS: Dict[str, float] = {
    "compression": 0.10, "fading_bounces": 0.08, "depth_drain": 0.10,
    "defender_pulling": 0.10, "attacker_stacking": 0.08, "flow_pressure": 0.10,
    "microprice_lean": 0.08, "velocity_rising": 0.07, "btc_alignment": 0.06,
    "repeated_tests": 0.05, "normalized_cvd": 0.06, "deep_obi": 0.07,
...
class LevelTracker:
    ...
    def __init__(self, thresholds: Optional[Thresholds] = None) -> None:
        ...
        self.closed_seen = 0
        self.swings_found = 0
        self._rebuild_key = None

    def rebuild(self, candles: List[Candle]) -> None:
        closed = [c for c in candles if c.closed]
        key = (len(closed), closed[-1] if closed else None)
        if key == self._rebuild_key:
            return
        self._rebuild_key = key
    ...
    def _measure_bounces(self, candles: List[Candle]) -> None:
        ...
        for level in self.levels:
            ...
            for candle in closed:
                ...
                if candle.start_ms < level.first_seen_ms:
                    continue
                if near:
                    if away and touched:
...
class PreBreakInputs:
    ...
    liquidation_component: float = 0.0
    ...
    def evaluate(self, direction: str, inputs: PreBreakInputs) -> PreBreakResult:
        ...
        if not inputs.book.synced:
            return PreBreakResult(direction=direction, level=level.price, probability=0.0,
                                  tests=level.tests,
                                  note="order book not synced; no model score offered")
        ...
        features = {
            ...
            "microprice_lean": clamp((1 if direction == LONG else -1) *
                (inputs.microprice_offset_bps or 0.0) / max(1e-9, (inputs.book.spread_bps or 0.0) / 2), 0.0, 1.0),
            "velocity_rising": self._velocity_rising(inputs.velocity_zscore,
                                                     inputs.velocity_acceleration) *
                clamp((1 if direction == LONG else -1) * inputs.flow_component, 0.0, 1.0),
            "btc_alignment": self._btc_alignment(direction, inputs.btc_lead_score),
            "repeated_tests": self._repeated_tests(level),
            "normalized_cvd": clamp((1 if direction == LONG else -1) * inputs.cvd_component, 0.0, 1.0),
            "deep_obi": clamp((1 if direction == LONG else -1) *
                               inputs.book.alignment.get("deep_book_score", 0.0), 0.0, 1.0),
            "liquidation_cascade": clamp((1 if direction == LONG else -1) *
                                         inputs.liquidation_component, 0.0, 1.0),
        }
