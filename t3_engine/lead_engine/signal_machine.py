...
class SignalSnapshot:
    ...
    def as_dict(self) -> Dict[str, object]:
        return {
            ...
            "level": self.level, "price": self.price,
            "model_score": round(self.break_probability, 2),
            "score_label": "MODEL_SCORE", "estimated_probability": None,
        }
...
class SignalMachine:
    ...
    def update(self, inputs: SignalInputs, now: Optional[float] = None) -> SignalSnapshot:
        ...
        confidence = max(0.0, min(1.0, inputs.confidence)) * 100.0 if inputs.healthy else 0.0
        if (state, direction, level) != (self.current.state, self.current.direction, self.current.level):
            snapshot = SignalSnapshot(
                ...
                changed_at=now, break_probability=probability, level=level, price=inputs.price,
            )
            self.history.append(replace(snapshot))
            ...
            self.current = replace(self.current, direction=direction,
                                   confidence=confidence, reason=reason,
                                   break_probability=probability, level=level)
    ...
    def _decide(self, inputs: SignalInputs, now: float):
        ...
        # Coverage and conflict gates precede EVERY directional state,
        # including reversal candidates. Confidence was previously unused.
        if inputs.confidence < thresholds.min_signal_confidence:
            return (WATCH, "", 0.0, "insufficient independent data coverage",
                    probability, level)
        if inputs.conflict_level == CONFLICT_HIGH or inputs.conflict >= MAX_CONFLICT:
            return (WATCH, "", 0.0, "independent layers disagree; signals off",
                    probability, level)
        ...
        if probability >= thresholds.a_plus_probability and pressure >= PRE_SIGNAL_PRESSURE:
            return (capped_top, direction, pressure,
                    ...
                    probability, level)
        if probability >= thresholds.high_probability and pressure >= PRE_SIGNAL_PRESSURE:
            return (HIGH_PROBABILITY, direction, pressure,
                    f"break score {probability:.0f}", probability, level)
        if probability >= thresholds.prebreak_probability and pressure >= WATCH_PRESSURE:
        ...
        if pressure >= PRE_SIGNAL_PRESSURE and level is not None and probability >= WATCH_PRESSURE:
