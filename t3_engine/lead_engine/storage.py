...
class Recorder:
    ...
    def __init__(self, engine, storage: "Storage",
        ...
        self._signal_keys: Dict[str, set] = {}
    ...
    def sweep_once(self) -> int:
        ...
        for symbol in list(self.engine.states):
            ...
            if state is None:
                continue
            try:
                frame = self.engine.get_state(symbol)
    ...
    def _record_transition(self, symbol: str, state) -> int:
        seen = self._signal_keys.setdefault(symbol, set())
        snapshots = list(state.signals.history) + [state.signals.current]
        written = 0
        keys = set()
        for current in snapshots:
            stamp = float(current.changed_at or 0.0)
            key = (stamp, current.state, current.direction, current.level)
            keys.add(key)
            if not stamp or key in seen:
                continue
            self.storage.record_signal(symbol, current.as_dict())
            seen.add(key)
            written += 1
        self._signal_keys[symbol] = seen & keys
        return written
