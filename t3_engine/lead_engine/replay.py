...
def events_from_messages(messages: Iterable[Dict[str, Any]]) -> List[ReplayEvent]:
    """Preserve capture arrival order; exchange timestamps do not order topics.

    Use captured receive timestamps when present. Older fixtures have only
    exchange timestamps, so latency cannot be recovered from them.
    """
    ...
    for message in messages:
        ...
        if not topic:
            continue
        stamp = int(message.get("_received_at_ms") or message.get("ts") or 0)
        if not stamp:
...
class Replay:
    ...
    def __init__(self, symbol: str, config: Optional[LeadEngineConfig] = None,
        ...
        replay_config = LeadEngineConfig(
            ...
            orderbook_depth=config.orderbook_depth, orderbook_topic_depth=config.orderbook_topic_depth,
            weights=config.weights, layer_weights=config.layer_weights,
            thresholds=config.thresholds, oi_poll_seconds=config.oi_poll_seconds,
            recompute_interval_seconds=config.recompute_interval_seconds,
        )
    ...
    def run(self, events: Sequence[ReplayEvent],
        ...
        for event in events:
            report.events += 1
            # Preserve captured arrival order and each stream's real age.
            replay_ms = max(replay_ms, event.timestamp_ms)
            message = dict(event.message, _received_at_ms=event.timestamp_ms)
            self.engine.handle_message(event.topic, message)
            parsed = parse_topic(event.topic)
            if parsed["symbol"] != self.symbol:
                other = self.engine.states.get(parsed["symbol"])
                if other:
                    other.snapshot(now=replay_ms / 1000.0, btc_state=self.engine._btc_state())
                continue
            ...
            frame = state.snapshot(now=replay_ms / 1000.0, btc_state=self.engine._btc_state())
            self.frames += 1
            report.frames += 1
            ...
            identity = (signal["state"], signal["direction"], signal.get("level"), signal["changed_at"])
            if identity != last_state and signal["state"] in ACTIONABLE:
                calls.append(Call(
                    symbol=self.symbol, state=signal["state"],
                    direction=signal["direction"] or "",
                    timestamp_ms=int(signal["changed_at"] * 1000), price=signal.get("price") or price,
    ...
    def _outcome(self, call: Call) -> None:
        ...
        complete = self.prices and self.prices[-1][0] >= call.timestamp_ms + self.horizon_ms
        call.resolved = moved if moved or complete else None
